...
class FileDAO(object):
    ...
    def __init__(self,title):
        ...
        self.dao = innerdao.Dao(path=path,initsqllist=sqllist)
    
    def addfile(self,afile):
        sql = '''INSERT INTO `files`(`name`,`path`,`isfile`,`mtime`,`parent`) 
                VALUES (?,?,?,?,?)
                '''
        list = {afile.name,afile.path,afile.isfile,afile.mtime,afile.prent}
    ...
    def addfiles(self,files,curpath = None):
        ...
        sql = '''INSERT INTO `tmpfiles`(`name`,`path`,`isfile`,`mtime`,`parent`) 
                VALUES (?,?,?,?,?)
                '''
        self.dao.savemany(sql, files, 100)
        #new files
    ...
    def gethotdir(self,fromtimestamp):
        sql = 'SELECT `name`,`path`,`isfile`,`mtime`,`id` FROM `files` WHERE `mtime`>?'
